							...
							<div class="accordion-inner">
								<table border='2' cellspacing='1' cellpadding='0' width=100%>									
									...
									<tr class="width100">
										...
										<td class='EWNormalGreenGeneral1 width25'>Initiating User Group</td>
										<td class='EWNormalGreenGeneral1 width25'>
											<select disabled id="InitUserGroup"  name="InitUserGroup" class="dropdown148px" onchange="setComboValueToTextBox(this,'wdesk:userGroup')">
											
											</select>
											<input type="hidden" name="wdesk:userGroup" id="wdesk:userGroup" value='<%=((WorkdeskAttribute)attributeMap.get("userGroup")).getValue()==null?"":((WorkdeskAttribute)attributeMap.get("userGroup")).getValue()%>'>
										</td>
									</tr>
									<%}%>
									<tr class="width100">
										<td class='EWNormalGreenGeneral1 width25'>Office Account</td>
										<td class='EWNormalGreenGeneral1 width25'>
											<input type='text' disabled name="wdesk:office_Account" id="wdesk:office_Account" value='<%=((WorkdeskAttribute)attributeMap.get("office_Account")).getValue()==null?"":((WorkdeskAttribute)attributeMap.get("office_Account")).getValue()%>'/>
										</td>
...
if(workstepName.equalsIgnoreCase("CSO_Initiate") || workstepName.equalsIgnoreCase("TT_Outward_Initition_checker") || workstepName.equalsIgnoreCase("TT_Outward_Initiator_reject"))
{
%>
...
%>
				...
				<!--Hidden fields according to the actors -->
					...
					<input type="hidden" name="wdesk:PAYMENT_REQ_MODE" id="wdesk:PAYMENT_REQ_MODE" value='<%=((WorkdeskAttribute)attributeMap.get("PAYMENT_REQ_MODE")).getValue()==null?"":((WorkdeskAttribute)attributeMap.get("PAYMENT_REQ_MODE")).getValue()%>'>
				<!--dec_Outward_Checker-->
					<input type="hidden" id="wdesk:dec_Outward_Checker" name="wdesk:dec_Outward_Checker" value='<%=((WorkdeskAttribute)attributeMap.get("dec_Outward_Checker")).getValue()==null?"":((WorkdeskAttribute)attributeMap.get("dec_Outward_Checker")).getValue()%>'>
					
				<!--dec_Outward_Reject-->
					<input type="hidden" id="wdesk:dec_Outward_Reject" name="wdesk:dec_Outward_Reject" value='<%=((WorkdeskAttribute)attributeMap.get("dec_Outward_Reject")).getValue()==null?"":((WorkdeskAttribute)attributeMap.get("dec_Outward_Reject")).getValue()%>'>
				...
				<table>
					...
					<input type="hidden" id="wdesk:scheme_type" name="wdesk:scheme_type" value='<%=((WorkdeskAttribute)attributeMap.get("scheme_type")).getValue()==null?"":((WorkdeskAttribute)attributeMap.get("scheme_type")).getValue()%>'>
		            <input type="hidden" id="wdesk:ibanFromEntityDetail" name="wdesk:ibanFromEntityDetail" value='<%=((WorkdeskAttribute)attributeMap.get("ibanFromEntityDetail")).getValue()==null?"":((WorkdeskAttribute)attributeMap.get("ibanFromEntityDetail")).getValue()%>'>
					...
					<input type="hidden" name="signedEFTCounter" id="signedEFTCounter" value=''>
					<input type="hidden" name="EFTformCounter" id="EFTformCounter" value=''>
...
<script src="jquery.min.js"></script>
		...
		function currencyFormat (object,decimalPoint) 
		{
			...
			if(lng > 0){
				if(num.indexOf(".") == -1) {
					...
					if((num.length > 11 || !num.match(fVal))) {
					...
					if(firstPart.length> 11 || !firstPart.match(fVal) || secondPart.length > decimalPoint || !secondPart.match(sVal))
					{
						if (x=="")
							alert("Please Enter Valid Transfer Amount. (Format: $$$$$$$$$)");
						else
		...
		function onDebitAccountChange(obj)
		{
			...
			var initiatingUserGroupVal = document.getElementById('wdesk:userGroup').value;
			var wi_name = document.getElementById("wdesk:wi_name").value;
			var isWICreatedByNewClick = '';
			
			var requestType='isWICreatedByNewClick';
			var xhr;
			if(window.XMLHttpRequest)
				 xhr=new XMLHttpRequest();
			else if(window.ActiveXObject)
				 xhr=new ActiveXObject("Microsoft.XMLHTTP");
			var url="/webdesktop/CustomForms/TT_Specific/HandleAjaxRequest.jsp";     
			var param="reqType="+requestType+"&winame="+wi_name;
			...
			if (xhr.status == 200 && xhr.readyState == 4)
			{
				...
				{
					alert("Some problem in fetching data from wi_history table.");
					return false;
				}
				if(parseInt(ajaxResult) == 1){
					isWICreatedByNewClick = "N";
				}else if(parseInt(ajaxResult) == 0){
					isWICreatedByNewClick = "Y";
				}
			}
			else
			{
				alert("Problem in fetching data from wi_history table.");
				return false;
			}
			
			if(initiatingUserGroupVal == '' && isWICreatedByNewClick == 'Y'){
				obj.value = ''; //clear debit account number.
				alert("Please select initiating user group first.");
				document.getElementById('InitUserGroup').focus();
				return; 
			}			
			//end.
			...
			//1) First Integration calls "Customer Details Fetch"
			var getCustSuccess = getCustomerDetails();
			var isOfficeAccount = document.getElementById("wdesk:office_Account").value;
			
			if(isOfficeAccount == "No" || isOfficeAccount == ""){
			if (getCustSuccess)
			{
			...
			}else if(isOfficeAccount == "Yes"){
				if (getCustSuccess)
				{
					//2) FCY_RATE Integration call
					var boolCal = calculateAmt();
					//not applicable for office accounts.
					/*if (boolCal != 'Error' && boolCal!=false)
					{
						//4) MemoPad Fetching from fincale and inserting into our DB
						var boolMemo = memopad(1);
...
function onOPSDataEntryFetchMemo() {
		...
		function initForm(workstepName,sMode)
		{
			...
			var currRemarksFromDB = "";
			currRemarksFromDB = ajaxRequest(workstepName,'SelectEnterRemarks');
			//alert("currRemarksFromDB: "+currRemarksFromDB);
			
			if(currRemarksFromDB == undefined)
				document.getElementById('wdesk:remarks').value = "";
			else
				document.getElementById('wdesk:remarks').value = currRemarksFromDB;
			
			/************************************/
			
			/************************************/
			//added by stutee.mishra
			var isViewSignClickedVal = "";
			isViewSignClickedVal = ajaxRequest(workstepName,'SelectViewSignClicked');
			//alert("isViewSignClickedVal: "+isViewSignClickedVal)
			/************************************/
			...
			if (workstepName == 'CallBack')
			{
				...
				//ajaxRequest(workstepName,'loggedInUserGroup');
				var savedInitiatedUserGroup = document.getElementById('wdesk:userGroup').value;
				var select = document.getElementById('InitUserGroup');
				var opt = document.createElement('option');
				opt.value = savedInitiatedUserGroup;
				opt.innerHTML = savedInitiatedUserGroup;
				opt.className="EWNormalGreenGeneral1";
				select.appendChild(opt);
				selectElement('InitUserGroup','wdesk:userGroup');
			...
			if (workstepName=='CSO_Initiate')
			{
				...
				ajaxRequest(workstepName,'isWICreatedByNewClick');
				...
				document.getElementById('EFT_Form').style.display = 'none';
				//change done by stutee.mishra
				var loggedInUser = document.getElementById('login_user').value;
				var reqType = 'loggedInUserGroup';
				var workstepname = "";
				var url = '/webdesktop/CustomForms/TT_Specific/HandleAjaxRequest.jsp?reqType='+reqType+"&loggedInUser="+loggedInUser;
				var xhr;
				var ajaxResult;
				var response;
				var x ="";
				...
				if (xhr.status == 200)
				{
					...
					if(response.indexOf('~') !== -1){
						groupArray = response.split('~');
						for (var i = 0; i< groupArray.length;i++)
						{
							var opt = document.createElement('option');
							opt.value = groupArray[i].trim();
							opt.title = groupArray[i].trim();
							opt.innerHTML = groupArray[i].trim();
							opt.length = groupArray[i].length;
							opt.className="EWNormalGreenGeneral1";
							select.appendChild(opt);
						}
						...
						var opt = document.createElement('option');
						opt.value = response.trim();
						opt.title = response.trim();
						opt.innerHTML = response.trim();
						opt.length = response.length;
						opt.className="EWNormalGreenGeneral1";
						select.appendChild(opt);
						select.value = response;
						document.getElementById('wdesk:userGroup').value = response;
					}
				}
				else
				{
					alert("Unable to fetch the groups of initiating user from the Master");
					return false;
				}
				selectElement('InitUserGroup','wdesk:userGroup');
				//end.
			...
			else if (workstepName=='TT_Outward_Initition_checker')
			{
				ajaxRequest(workstepName,'CurrencyDropDown');				
				ajaxRequest(workstepName,'CountryDropDown');				
				ajaxRequest(workstepName,'dropDownBenefBankCode');
				...
				selectElement('authorized_sign','wdesk:authorized_sign');
				
				selectElement('InitUserGroup','wdesk:userGroup');
				selectElement('transtype','wdesk:trans_type');//Transaction type
				//populate transaction code for business transaction type.
				...
				document.getElementById("wdesk:debt_acc_num").disabled=true;
				document.getElementById("app_received").disabled=true;
				document.getElementById("authorized_sign").disabled=true;
				document.getElementById("wdesk:chargesCombo").disabled=true;
				document.getElementById("wdesk:pref_rate").disabled=true;
				document.getElementById("wdesk:deal_ref_code").disabled=true;
				document.getElementById("btnValDealRef").disabled=true;
				document.getElementById("transcode").disabled=true;
				document.getElementById("wdesk:trans_amt").disabled=true;
				document.getElementById("transamtcurr").disabled=true;
				document.getElementById("remitamtcurr").disabled=true;
				document.getElementById("transtype").disabled=true;
				document.getElementById("wdesk:purp_of_payment1").disabled=true;
				document.getElementById("wdesk:purp_of_payment2").disabled=true;
				document.getElementById("wdesk:purp_of_payment3").disabled=true;
				document.getElementById("getCustDetails").disabled=true;
				document.getElementById("view_sign").disabled=true;
				document.getElementById("getFxRate").disabled=true;
				document.getElementById("getAccBalance").disabled=true;
				document.getElementById("wdesk:benef_name").disabled=true;
				document.getElementById("wdesk:middleName").disabled=true;
				document.getElementById("wdesk:beneficiaryCity").disabled=true;
				document.getElementById("wdesk:lastName").disabled=true;
				document.getElementById("wdesk:iban").disabled=true;
				document.getElementById("countryOfResCombo").disabled=true;
				document.getElementById("wdesk:benefBankName").disabled=true;
				document.getElementById("benefBankCntryCombo").disabled=true;
				document.getElementById("wdesk:benefBankBranch").disabled=true;
				document.getElementById("wdesk:benefCityState").disabled=true;
				document.getElementById("dropDownBenefBankCode").disabled=true;
				document.getElementById("wdesk:benefActualCode").disabled=true;
				document.getElementById("wdesk:interBankName").disabled=true;
				document.getElementById("wdesk:interBankBranch").disabled=true;
				document.getElementById("dropDownInterBankCode").disabled=true;
				document.getElementById("wdesk:interBankCntryCombo").disabled=true;
				document.getElementById("wdesk:interCityState").disabled=true;
				document.getElementById("wdesk:interActualCode").disabled=true;
				document.getElementById('getAccBalance').style.display = 'none';
				document.getElementById('getFxRate').style.display = 'none';
				
			}
			
			else if (workstepName=='TT_Outward_Initiator_reject')
			{
				ajaxRequest(workstepName,'CurrencyDropDown');				
				ajaxRequest(workstepName,'CountryDropDown');				
				ajaxRequest(workstepName,'dropDownBenefBankCode');
				...
				selectElement('transtype','wdesk:trans_type');//Transaction 
				//populate transaction code for business transaction type.
				var select = document.getElementById('transcode');
				while (select.firstChild) {
					select.removeChild(select.firstChild);
				...
				document.getElementById("wdesk:debt_acc_num").disabled=true;
				document.getElementById("EFT_Form").disabled=true;
				document.getElementById("InitUserGroup").disabled=true;
				document.getElementById('getAccBalance').style.display = 'none';
				document.getElementById('getFxRate').style.display = 'none';
			    document.getElementById("view_sign").disabled=true;
			...
			{
				//change done by stutee.mishra to auto-populate callback remarks if case is office account.
				var isOfficeAccount = document.getElementById("wdesk:office_Account").value;
				if(isOfficeAccount == "Yes"){
					document.getElementById("wdesk:callbackwaiver").value = "DEBIT ON OFFICE ACCOUNT"
				}
				//end.
			...
			if (workstepName!= 'PostCutOff_Init' &&  workstepName!='CSO_Initiate' &&  workstepName!='Ops_DataEntry' && workstepName!='Ops_Hold' && workstepName!='Ops_Hold' && workstepName!='Error' )
			{
				...
				if(isanyExcp!=null && isanyExcp!="" && isanyExcp.indexOf("~[Raised")>-1 && isViewSignClickedVal!='Yes')
...
function openCustomDialog (dialogToOpen,workstepName)
		{
			...
			var currRemarksF = 'default value';
			if (workstepName!=null &&  workstepName!='')
			{
				...
				if (dialogToOpen=='View Sign')
				{
					...
					if (debt_acc_num!=null && debt_acc_num!='')
					{
						...
						sOptions = 'left=300px,top=200px,width=600px,height=600px,scrollbars=1,resizable=1; center:yes;edge:raised; help:no; resizable:no; scroll:yes;scrollbar:yes; status:no; statusbar:no; toolbar:no; menubar:no; addressbar:no; titlebar:no;';
						...
						ajaxRequest(workstepName,'isViewSignClicked');
						...
						if (window.showModalDialog) {
							if (workstepName=='Ops_Checker' || workstepName=='Ops_Checker_DB'){
								...
								if(isOfficeAccount == 'Yes')
									url = "/webdesktop/CustomForms/TT_Specific/OpenImageOfficeAcc.jsp?isOfficeAccount="+isOfficeAccount+"&workstepName="+workstepName+"&signMatchNeededAtChecker="+signMatchNeededAtChecker+"&debt_acc_num="+debt_acc_num
								else
									url = "/webdesktop/CustomForms/TT_Specific/OpenImage.jsp?workstepName="+workstepName+"&signMatchNeededAtChecker="+signMatchNeededAtChecker+"&debt_acc_num="+debt_acc_num
								
								popupWindowSign = window.showModalDialog(url,"_blank",sOptions);
							
							}
							else {
								
								if(isOfficeAccount == 'Yes')
									url = "/webdesktop/CustomForms/TT_Specific/OpenImageOfficeAcc.jsp?isOfficeAccount="+isOfficeAccount+"&workstepName="+workstepName+"&debt_acc_num="+debt_acc_num
								else
									url = "/webdesktop/CustomForms/TT_Specific/OpenImage.jsp?workstepName="+workstepName+"&debt_acc_num="+debt_acc_num
								popupWindowSign = window.showModalDialog(url,"_blank",sOptions);
							}
						}else{
							if (workstepName=='Ops_Checker' || workstepName=='Ops_Checker_DB'){
								...
								if(isOfficeAccount == 'Yes')
									...
									url = "/webdesktop/CustomForms/TT_Specific/OpenImage.jsp?workstepName="+workstepName+"&signMatchNeededAtChecker="+signMatchNeededAtChecker+"&debt_acc_num="+debt_acc_num
								popupWindowSign = window.open(url,"_blank",windowParams);
					...
					window.parent.mainSave();
					ajaxRequest(workstepName,'EnterRemarks');
					//var currRemarks = document.getElementById('wdesk:remarks').value;
					//alert("currRemarks: "+currRemarks);
					var wi_name = '<%=wi_name%>';
					var H_CHECKLIST = document.getElementById('wdesk:H_CHECKLIST').value;
				...
				{
					var wi_name = '<%=wi_name%>';
					var ArchivalMode=window.parent.ArchivalMode;   //added by Sowmya for Cabinet Archival WIs
					...
					if (window.showModalDialog) {
						//window.showModalDialog("history.jsp?WINAME="+wi_name,"", "dialogWidth:60; dialogHeight:400px; center:yes;edge:raised; help:no; resizable:no; scroll:yes;scrollbar:yes; status:no; statusbar:no; toolbar:no; menubar:no; addressbar:no; titlebar:no;");
						window.showModalDialog("history.jsp?WINAME="+wi_name+"&ArchivalMode="+ArchivalMode,"", "dialogWidth:60; dialogHeight:400px; center:yes;edge:raised; help:no; resizable:no; scroll:yes;scrollbar:yes; status:no; statusbar:no; toolbar:no; menubar:no; addressbar:no; titlebar:no;");
					} else {
						//window.open("history.jsp?WINAME="+wi_name,"",windowParams);
						window.open("history.jsp?WINAME="+wi_name+"&ArchivalMode="+ArchivalMode,"",windowParams);
					}
					/************************************************************/
					//added below to handle window.open/window.showModalDialog according to type of browser  ends here.
		...
		function getCustomerDetails()
		{
			...
			if (debt_acc_num!=null && debt_acc_num!="")
			{
				document.getElementById("wdesk:acc_bal").value = '';
				var requestType='GET_CUSTOMER_DETAILS';
				var wi_name = '<%=wdmodel.getWorkitem().getProcessInstanceId()%>';
		...
		function setCustomerDetailsOnForm(ajaxResult)
		{
			...
			var schemeTypeInResponse = "", isOfficeAccCheck1 = "", isOfficeAccCheck2 = "";
			if(strCode == '0000')
			{
				//Added by stutee.mishra for schem type. 
			    schemeTypeInResponse = xmlDoc.getElementsByTagName("SchemeType")[0].childNodes[0].nodeValue;
				if (schemeTypeInResponse!=null && schemeTypeInResponse!="")
				{
					var requestType='VALIDATE_SCHEME_TYPE_OFFICE_ACC';
					var xhr;
					if(window.XMLHttpRequest)
						 xhr=new XMLHttpRequest();
					else if(window.ActiveXObject)
						 xhr=new ActiveXObject("Microsoft.XMLHTTP");
					var url="/webdesktop/CustomForms/TT_Specific/HandleAjaxRequest.jsp";     
					var param="schemeTypeInResponse="+schemeTypeInResponse+"&reqType="+requestType;
					xhr.open("POST",url,false);
					xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');		
					xhr.send(param);
					if (xhr.status == 200 && xhr.readyState == 4)
					{
						...
						if(ajaxResult.indexOf("Exception")==0)
						{
							alert("Some problem in fetching scheme_type from master.");
							return false;
						}
						if(parseInt(ajaxResult) > 0){
							isOfficeAccCheck1 = "Y";
						}else{
							isOfficeAccCheck1 = "N";
						}
						document.getElementById("wdesk:scheme_type").value = schemeTypeInResponse;
					}
					else
					{
						alert("Problem in fetching scheme_type from master.");
						return false;
					}
				}
				else 
				...
				//check 2 for debit account number.
				if(isOfficeAccCheck1 == "Y"){
					var debitAccNumOnForm = document.getElementById('wdesk:debt_acc_num').value;
					var userGroupOnForm = document.getElementById("wdesk:userGroup").value;
					var requestType='VALIDATE_DEBIT_ACC_NUM';
					var xhr;
					if(window.XMLHttpRequest)
						 xhr=new XMLHttpRequest();
					else if(window.ActiveXObject)
						 xhr=new ActiveXObject("Microsoft.XMLHTTP");
					var url="/webdesktop/CustomForms/TT_Specific/HandleAjaxRequest.jsp";     
					var param="debitAccNumOnForm="+debitAccNumOnForm+"&userGroupOnForm="+userGroupOnForm+"&reqType="+requestType;
					xhr.open("POST",url,false);
					xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');		
					xhr.send(param);
					if (xhr.status == 200 && xhr.readyState == 4)
					{
						...
						if(ajaxResult.indexOf("Exception")==0)
						{
							alert("Some problem in fetching data from office account mapping master.");
							return false;
						}
						if(parseInt(ajaxResult) > 0){
							document.getElementById("wdesk:office_Account").value = "Yes";
							//autopopulate fields
							document.getElementById("app_received").value = "Yes";
							document.getElementById("app_received").disabled = true;
							document.getElementById("wdesk:isOriginalReceived").value = "Yes";
							document.getElementById("authorized_sign").value = "Yes";
							document.getElementById("authorized_sign").disabled = true;
							document.getElementById("wdesk:authorized_sign").value = "Yes";
							document.getElementById("transtype").value = "Business";
							document.getElementById("wdesk:trans_type").value = "Business";
							//populate transaction code for business transaction type.
							var select = document.getElementById('transcode');
                            while (select.firstChild) {
								select.removeChild(select.firstChild);
							...
							selectElement('transcode','wdesk:trans_code');
							//save iBAN in hidden field from entity_detail API call for office_accounts
							document.getElementById("wdesk:ibanFromEntityDetail").value = xmlDoc.getElementsByTagName("IBANNumber")[0].childNodes[0].nodeValue;
							
						}else{
							alert("User does not have rights to initiate transaction for this office account.");
						    return false;
						}
					}
					else
					{
						alert("Problem in fetching office account mapping master.");
						return false;
					}
				}else{
					document.getElementById("wdesk:office_Account").value = "No";
				}
				//end.
				...
				//handling of office account cases for customer type by stutee.mishra
				var CustTypeCode=""; var CustTypeValue="";
				if(document.getElementById("wdesk:office_Account").value == "No"){
					CustTypeCode=xmlDoc.getElementsByTagName("CustomerType")[0].childNodes[0].nodeValue;
		...
		function getTTRefNum()
		{
			...
			if(TTRefNum==null || TTRefNum=="")
			{
				...
				var isOfficeAccount = document.getElementById("wdesk:office_Account").value;
				var ibanFromEntityDetail = document.getElementById("wdesk:ibanFromEntityDetail").value;
		...
		//alert('req type'+ req);
			...
			//alert('inside 2');
				...
				ajaxResult=  ajaxResult.replace('<?xml version="1.0"?>','');
				if(ajaxResult.indexOf('&quot;')>-1)
				{
				ajaxResult=  ajaxResult.split('&quot;').join('"');//added by stutee.mishra on 13/02/2022
				}
				//ajaxResult=  ajaxResult.split('\n').join('');
					...
					//alert('trtags 2'+ trTags.length);
					for (var i = 0; i < trTags.length; i++) 
					{
						//alert(trTags[i].innerHTML);
						var x = trTags[i].innerHTML.split('@');
						
						//alert("value of x: "+x);
						//alert(x.length);
						for(var y = 0; y < x.length-1; y++){
							var row = document.getElementById("memo_pad").insertRow(y+1);
					//	alert(trTags[i].innerHTML);
						    //trTags[i].innerHTML=trTags[i].innerHTML.split('\n').join('');
							...
							for(var z=0;z<=1;z++){
								...
								if(z==0){
									...
									//Changes done by stutee.mishra
									var desc = ""; var descArr = "";
									if(x[y].indexOf('>')>-1){
										descArr = x[y].split('>');
										desc = descArr[3];
									}	
		...
		function changeVal(dropdown,workstepName)
		{
			if (workstepName == "CSO_Initiate")
				...
				document.getElementById("wdesk:dec_Outward_Checker").value = dropdown.value;
			else if (workstepName == "TT_Outward_Initiator_reject")
				document.getElementById("wdesk:dec_Outward_Reject").value = dropdown.value;
		...
		function getFcyRate(opt)
		{
			...
			else
			{
				...
				var param="wi_name="+wi_name+"&cif="+cif+"&remitCurrType="+remitCurrType+"&debt_acc_num="+debt_acc_num+"&transCurrType="+transCurrType+"&debitCurrType="+accCurrType+"&tranAmountWithoutComma="+tranAmountWithoutComma+"&requestType="+requestType+"&isbankemployee="+isbankemployee+"&isOfficeAccount="+isOfficeAccount;
				xhr.open("POST",url,false);
				xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');		
				xhr.send(param);
				...
				if (channel_id == 'IB' || channel_id=='MB' || channel_id=='YAP' || channel_id=='DIP') {
					...
					if (select.hasChildNodes()) {
						...
						for(var x = 0; x<select.childNodes.length; x++){
							if(select.childNodes[x].value =='Call Back Waiver Request' || select.childNodes[x].value =='Refer'){
								select.removeChild(select.childNodes[x]);
							}
						}
		...
		//vedant
	function onEFTForm(){
			var requestType='EFT_form_generate';
			var wi_name = '<%=wdmodel.getWorkitem().getProcessInstanceId()%>';
			var xhr;
			if(window.XMLHttpRequest)
				 xhr=new XMLHttpRequest();
				 ...
				 xhr=new ActiveXObject("Microsoft.XMLHTTP");
			var url="/webdesktop/CustomForms/TT_Specific/EFTForm.jsp";     
			var param="wi_name="+wi_name+"&requestType="+requestType;
			xhr.open("POST",url,false);
			xhr.setRequestHeader('Content-Type','application/x-www-form-urlencoded');		
			xhr.send(param);
			if (xhr.status == 200)
			{
				...
				alert("Problem in getting EFT form.");
				return false;
			}
